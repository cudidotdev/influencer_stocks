...
const quickBuyFormSchema = z.object({
  shares: z.coerce
    .number()
...
type QuickBuyFormValues = z.infer<typeof quickBuyFormSchema>;

export function QuickBuyForm({ stockId }: { stockId: number }) {
  ...
  const form = useForm<QuickBuyFormValues>({
    resolver: zodResolver(quickBuyFormSchema),
    defaultValues: {
      shares: 1,
      total_price: 0,
      slippage: 5,
  ...
  async function handleSubmit(values: QuickBuyFormValues) {
    ...
    try {
      ...
      const msg = msgComposer.quickBuy(
        {
        ...
        [
          {
            amount: Math.floor(
              values.total_price * 1_000_000 * (1 + values.slippage / 100),
            ).toString(),
            denom: "uhuahua",
          },
        ],
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Amount of percentage you are willing to exceed the shown
